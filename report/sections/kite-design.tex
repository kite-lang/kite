...
For each declared function, it is possible to declare its type. For instance the function \code{length} has the following type: \[ length: [a] \to Int \]

The syntax of type declaration is in general as the example in \ref{sec:func-decl} and thus \code{length} should have a type declaration as follows:
...
The type-checker checks whether the declared type matches the type inferred from the actual function, and yields a TypeError if it is declared wrongly.

TODO: Er det lame at have nedenstående med?

It is important to note that type declarations, should be made top level, as is the case with all declarations.
