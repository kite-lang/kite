...
TODO: Implicit return, blocks - Meget kort beskrevet under
functions. Mere?
...
Strings is represented as syntactic sugar for lists of characters. For
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|l|}
    ...
    $powmod(x, y, m)$ & Modulo exponentation \\ \hline
  \end{tabular}
...
A function is defined as a list of arguments and a block. A block is
defined as a list of expressions, where it is implicit that the last
expression is returned TODO: reference til implementation?. Each block
has its own immediate scope.
...
A runnable program must always include the `main' function, which will
be called once when the emitted code is run (by
Node.js~\footnote{http://nodejs.org/} if target code generation
...
functions is syntactic sugar for prefix function calls, with some
...
An example of pattern matching on lists is our implementation of the function \code{reverse}~\ref{foundation}, which matches `xs' with an expression, empty list, and the head and tail of `xs':
...
It should be noted that the addition operator, `+', has type \code{a
...
Kite supports exceptions using the keyword \code{panic}. \code{panic}
will stop Kite execution.
...
\end{kite}
Where if a user tries to take the head of an empty list, Kite
execution will stop and the error message will be shown.
TODO: Lidt random...


\subsection{Foundation}
As mentioned earlier in the \nameref{sec:requirements}, the `Foundation' of Kite is a standard library implemented in Kite consisting of often used functions. Kite's Foundation is inspired by Haskell's `Prelude', and includes many functions found in
Prelude. 
