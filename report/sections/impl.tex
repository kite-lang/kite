...
Where the emitter takes a \code{PLambda} node with its given parameters and body. There will therefore be emitted a JavaScript function with the given parameters and body, where the body, of type \code{PBlock}, recursively emitted. The \code{PBlock} node, which is usually the body of a \code{PLambda}, is emitted as follows:

\begin{haskell}
emit (PBlock exprs) = emitAll ";" exprs
\end{haskell}

The \code{PBlock} is a list of expressions, which is emitted recursively separated with semi-colons (\code{;}).
...
\end{haskell}
Here, the identifier is declared as JavaScript variable with the keyword \code{var} and assigned to an expression.
...
The code generator will emit the following, where \code{elem} (from Foundation) is partially applied:
