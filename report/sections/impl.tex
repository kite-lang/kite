...
The implementation of code generation focuses on JavaScript (JS), as the
initial aim of LLVM code generation was not met. More on this will be
discussed in \nameref{sec:discussion}. 

The codegen module will run recursively through the AST, received from the
parser, and emit JS code for each AST node. 

Examples of emitted code from a given AST node:
The \code{PLambda} node, which is a function declaration, will be
emitted to the following JS:
\begin{haskell}
  emit (PLambda param body) = printf "(function(%s) {%s})" param (emit body)
\end{haskell}
Where the emitter takes a \code{PLambda} with given parameters and
body. The codegen module will then emit a JS function with the given
parameters and go through the body. 

The \code{PBlock} node, which is usually the body of a \code{PLambda},
is emitted as follows:
\begin{haskell}
  emit (PBlock exprs) =
  emitAll ";" exprs
\end{haskell}
The \code{PBlock} takes a list of expressions, and emits the
expressions with a \code{;} separating them.

The \code{PBind} node, which binds an ID to a given expression, is emitted
as follows:
\begin{haskell}
  emit (PBind ide expr) =
...
The JS codegen module will also embed \code{kt\_runtime.js} (see
\ref{kt-runtime}), which is Kites JS runtime. The runtime includes
various JS functions, which can be used directly in Kite.  As an example, basic I/O
is included in \code{kt\_runtime.js}. Basic I/O includes print and
