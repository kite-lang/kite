...
This can be run command-line with: \code{\$ kite --target=javascript SOMETHING/DARKSIDE.kite | node} TODO: fix the filepath
...
We have made some benchmarking of Kite + Node.js, Python and Haskell. In the benchmarking the programs have run single-threaded.
...
It is clear that Kite + Node.js is overall slower and scales worse than the two other languages. The lack of good scaling is likely due to the fact that the recursive call stack of \code{flatMap} in list comprehensions becomes larger with larger lists. We will discuss this as a potential optimization in 
...
The variable \code{l} is the shuffled list, and \code{s} is the sorted one. Kite implements Quicksort, which on average has time complexity O($n log(n)$).
